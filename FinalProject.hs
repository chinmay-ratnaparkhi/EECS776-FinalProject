...
-- Draw a Car between two points, top left and bottom right. Also uses the rectDraw method.
...
filterPossibleCollisions (x:xs)= if a < 140 && a > -30
                                 then x : filterPossibleCollisions xs
...
collides plr [] = False
collides plr (x:xs) = if position plr == lane x 
                      then True
...
listDraw [] f = stroke ()
listDraw ((RectCoord (p1, p2) (q1, q2)):xs) f = 
    do
        if (mod' p2 40 == 0)
        then do     
...
draw pressed gameState= 
            do
                ...
                if (state (player gameState)) == Alive
                then do
                    if (score gameState) `mod` 20 < 10
                        then do
                            drawRoad roadTuples 210 214
                            drawRoad roadTuples 335 339
                        else do
                            drawRoad altRoadTuples 210 214
                            drawRoad altRoadTuples 335 339
                    -- Player car 
...
mainControl context gameState= do
        ...
        let imdState = if (state (player gameState)) == Alive 
            then Game { player= (getCar (position (player gameState)) getVal), enemies= moveEnemies 10 (enemies gameState), score= 1+(score gameState)}
            else gameState
        ...
        let newState = if collides (player imdState) (filterPossibleCollisions (enemies imdState))
                       then Game { player= MainCar {state =Dead, position= position (player imdState),
                                 ...
                                 enemies= enemies imdState,
                                 score = score imdState
                            }
                       else imdState
        {-if (score gameState `mod` 5 == 0)
                                             then moveEnemies 5 (enemies gameState)
                                             else (enemies gameState) -}
        --threadDelay (10 * 1000)
        loop context getVal newState


messageToTerm :: (Eq a, Show a) => Maybe a -> IO ()
messageToTerm x = if x == Nothing then return () else putStrLn $ "And I have " ++ show (x)



main = blankCanvas 3000 { events = ["keyup","keydown"] } $ \ context -> 
    do
        g <- getStdGen
        let gameState = Game {
                                player = (MainCar{  position= Mid, 
                                                    state= Alive, 
                                                    body= (rectDraw ((RectCoord (50, 175) (150, 225)), 
                                                            brickDraw))
                                                    }
                                        ), 
                                        score=0,
